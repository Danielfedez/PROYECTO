...
Para acceder al documento con el código, clica [aquí](Pregunta_2_code.ipynb)

En el primer informe vimos que los tres puestos más populares son *Data Analyst*, *Data Engineer* y *Data Scientist*. Estos puestos solicitan diversas habilidades, y sería interesante saber cuáles son las más solicitadas.
...
```
En nuestro *dataset*, la columna `job_skills` contiene listas a modo de input, así que para cada `job_title_short` hay una lista asociada con las habilidades solicitadas. Por ello, y para facilitar el tratamiento de esta columna, usamos el método `explode` para transformar estas listas en filas independientes para cada rol y habilidad, y `ast.literal_eval` para eliminar las filas vacías.
```
import ast
